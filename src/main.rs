...
impl SeasonPage {
    ...
    fn maybe_find_gamelogs(page: &Document, table_name: &str) -> Vec<Gamelog> {
    ...
    pub fn gamelogs(&mut self) -> Option<Vec<Gamelog>>{
        ...
        let gamelogs = Vec::new();
        if let Some(reg_gamelogs) = self.maybe_find_gamelogs(&page, "pgl_basic") {
            gamelogs.push_all(reg_gamelogs)
        }
        if let Some(playoff_gamelogs) = self.maybe_find_gamelogs(&page, "pgl_basic_playoffs") {
            gamelogs.push_all(playoff_gamelogs)
        }
...
impl GamelogTable {
    ...
    pub fn gamelogs(&self) -> Option<Vec<Gamelog>> {
        let names = vec!["rk", "num", "date", "team", "date", "is_home", "opp", "result", "min",
                         "fg", "fga", "tp", "tpa", "ft", "fta", "orb", "drb", "ast", "stl", "blk",
                         "tov", "pf", "pts", "margin"];
    ...
    fn parse_col(index: usize, col: Selection) -> Stat {
        ...
        match &index {
            ...
            7 => Stat::Margin(text_option.map(|t| GAME_RESULT_RE.find(t).first()) as usize),
            8 => Stat::Bool(text_option.map(|x| x != "1")),
            9 => Stat::MaybeNum(Self::remove_empty(text_option).map(|x| x.split(":").first())),
    ...
    fn remove_empty(text_option: Option<String>) -> Option<usize> {
        text_option.and_then(|text| if let text = "" { None } else { Some(text as usize) })
    }
}
